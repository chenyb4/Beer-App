<script>
    import {Sidebar, SidebarWrapper, SidebarGroup, SidebarBrand, DarkMode} from "flowbite-svelte";
...
<Sidebar {activeUrl} {activeClass} {nonActiveClass} asideClass="flex h-screen">
    <SidebarWrapper divClass="bg-light-s_bg dark:bg-dark-s_bg h-full w-52">
        <SidebarGroup>
            ...
            <NavBarOption title={$t("homepage.inventory_management.title")} site="/inventory_management"/>
            <NavBarOption title={$t("homepage.administration.title")} site="/administration" />
            <NavBarOption title={$t("homepage.transaction_history.title")} site="/transaction_history"/>
            <NavBarOption title={$t("homepage.inventory_history.title")} site="/inventory_history"/>

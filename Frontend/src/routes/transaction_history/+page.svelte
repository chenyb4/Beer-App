<script>
  ...
  let currentPage = 1;
  const pages =
    Math.ceil(data.allOrders?.meta.total / data.allOrders?.meta.page_size) || 1;
...
<TablePage
  ...
  {currentPage}
  changeData={changeTransactionHistory}
  {pages}
>
  <TableHeader
    headerValues={[
      ...
      {#each allOrders as entry}
        <TableBodyRow>
          ...
          <TableCell position="middle"
            >{dateToStringWithTime(entry.createdAt)}</TableCell
          >
